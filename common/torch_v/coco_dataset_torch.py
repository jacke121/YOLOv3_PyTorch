...
from common.torch_v import data_transforms_torch
CLASSES = ('mouse',)
    ...
    def run(self):#定义每个线程要运行的函数
      for index in range(len(self.listDataset.img_files)):
          if index in self.listDataset.img_d:
              ...
              img = self.listDataset.loader(img_path)
              if img is None:
...
class COCODataset(Dataset):
    def __init__(self, list_path, img_size, is_training, is_debug=False,data_size=20000,is_scene=False):
        ...
        self.transforms = data_transforms_torch.Compose()
        # if is_training:
        #     self.transforms.add(data_transforms.ImageBaseAug())
        # self.transforms.add(data_transforms.KeepAspect())
        self.transforms.add(data_transforms_torch.ToTensor(self.img_size,self.max_objects, self.is_debug))

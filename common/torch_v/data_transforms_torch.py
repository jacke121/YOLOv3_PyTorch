...
class ToTensor(object):
    def __init__(self,img_size, max_objects=50, is_debug=False):
        self.max_objects = max_objects
        self.is_debug = is_debug
        self.transforms=transforms.Compose([transforms.Resize(size=img_size, interpolation=3),
                         transforms.ToTensor(),
                         transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])]) # Image.BICUBIC
    ...
    def __call__(self, sample):
        ...
        return {'image': image, 'label': torch.from_numpy(filled_labels),'img_path':sample['img_path']}

...
def test(config):
    ...
    net = net.cuda()
    ini_files = [inifile for inifile in os.listdir(os.path.join(config['test_weights'], 'result')) if inifile.endswith('.ini')]
    ...
    config_file_path = os.path.join(config['test_weights'], 'result',ini_files[max_index])
    Bi_picpath = os.path.join(config['test_weights'], 'result',ini_files[max_index]).replace('.ini','')
    ...
    weight_file = os.path.join(config['test_weights'],'%s.weights'%ini_files[max_index].split('_')[0])
    ...
    for _jpg_images in err_jpgfiles:
        ...
        for idx, detections in enumerate(batch_detections):
            ...
            cv2.imwrite(os.path.join(Bi_picpath,'%s.jpg'%os.path.basename(pic_name)),image_show)
